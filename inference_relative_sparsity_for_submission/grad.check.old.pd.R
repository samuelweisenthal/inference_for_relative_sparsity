...
(round(grad(mi.pd,x=beta,b=b,Ss=Ss,As=As,gamma=gamma,Rs=Rs,u=u),7)
  ==round(dbetami.pd(beta,b,Ss,As,Rs,gamma,u=u),7))
(round(hessian(mi.pd,x=beta,b=b,Ss=Ss,As=As,gamma=gamma,u=u,Rs=Rs),7)
  ==round(ddbetami.pd(beta=beta,b=b,Ss=Ss,As=As,Rs=Rs,u=u,gamma=gamma),7))
...
b=b0#matrix(c(-1))
round(grad(kappa,x=beta,Ss=Ss,As=As),7)==round(dthetakappa(beta,Ss,As),7)
gradmi.nd = grad(mi.pd,x=beta,b=b,Ss=Ss,As=As,gamma=gamma,Rs=Rs,u=u)
gradmi = dbetami.pd(beta,b,Ss,As,Rs,gamma,u)
round(gradmi.nd,7)==round(gradmi,7)
(round(hessian(mi.pd,x=beta,b=b,Ss=Ss,As=As,gamma=gamma,Rs=Rs,u=u),7)
  ==round(ddbetami.pd(beta=beta,b=b,Ss=Ss,As=As,Rs=Rs,gamma=gamma,u=u),7))
grad.li = dli(Ss,As,b) 
...
if(K>1){
  cr=round(dbdbetami.pd(beta,b,Ss,As,Rs,gamma,u),5)
  ...
  mi.ind.11 = function(beta1,b1){
    mi.ind.pd(beta1,beta2,b1,b2,Ss,As,gamma,Rs,u)
  }
...
# # for(i in 1:NC){
# #   thresh =1e-6
# #   lines(1:T,sss[[i]],col=(1+(abs(gd[i])>thresh)),lwd=1)#(1+(abs(gd[i])>thresh)))
...
#   thresh =1e-6
#   if (abs(gd[i])>thresh){
#   lines(1:T,sss[[i]],col=rgb(1,0,0,alpha=0.1),lwd=1)
#   }
#   #col=rgb(10*abs(ncd[i]),0,0))
# }
# 
...
#   thresh =1e-6
#   if (abs(gd[i])<=thresh){
#     lines(1:T,sss[[i]],col=rgb(0,0,0,alpha=0.1),lwd=1)
#   }
#   #col=rgb(10*abs(ncd[i]),0,0))
# }
# 
